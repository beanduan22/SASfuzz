...
if "Model" not in ns or "make_inputs" not in ns:
    print("SKIP:missing Model or make_inputs")
    sys.exit(0)

try:
    raw_inputs = ns["make_inputs"]()
    double_inputs = []
    for x in raw_inputs:
        if isinstance(x, torch.Tensor) and x.is_floating_point():
            double_inputs.append(x.detach().double().requires_grad_(True))
        else:
            double_inputs.append(x)

    diff_idx = [i for i, x in enumerate(double_inputs)
                if isinstance(x, torch.Tensor) and x.requires_grad]
    if not diff_idx:
        print("SKIP:no differentiable inputs")
        sys.exit(0)

    model = ns["Model"]().cpu().eval()
    model = model.double() if hasattr(model, "double") else model

    diff_inputs = tuple(double_inputs[i] for i in diff_idx)

    def f(*args):
        full = list(double_inputs)
        for pos, idx in enumerate(diff_idx):
            full[idx] = args[pos]
        out = model(*full)
        if isinstance(out, (list, tuple)):
            return torch.stack([o.sum() if isinstance(o, torch.Tensor) else torch.tensor(o)
                                for o in out]).sum()
        return out.sum()

    ok = torch.autograd.gradcheck(
        f, diff_inputs, raise_exception=False, eps=1e-4, rtol=1e-2, atol=1e-5,
    )
    print("PASS" if ok else "FAIL:Jacobian mismatch")
except Exception:
    print(f"ERROR:{traceback.format_exc(limit=3)}")
"""
...
def run_gradcheck(
    ...
    code_path = workspace / f"gc_model_{model_id:04d}.py"
    runner_path = workspace / f"gc_runner_{model_id:04d}.py"
    code_path.write_text(model_code)
    runner_path.write_text(_GRADCHECK_RUNNER)
    ...
    try:
        ...
        return f"gradcheck timed out after {timeout}s"
    except Exception as exc:
        return f"gradcheck subprocess error: {exc}"

    stdout = (proc.stdout or "").strip()
    if stdout.startswith("PASS") or stdout.startswith("SKIP"):
        return None
