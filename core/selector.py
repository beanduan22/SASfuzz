...
class MultiRouletteSelector:
    def __init__(
        ...
        state_signals: Optional[StateSignals] = None,
    ) -> None:
        ...
        self._signals: Optional[StateSignals] = state_signals

    def select(self, n: int = 30, dimension: str = "") -> List[str]:
        return self.select_plan(n=n, dimension=dimension).all_apis

    def select_plan(self, n: int = 30, dimension: str = "", **_: object) -> SelectionPlan:
        groups = self._groups
        ...
        for g, apis in groups.items():
            ...
            if k <= 0:
                continue
            selected.extend(self._roulette(list(apis), k, dimension=dimension))
    ...
    def _roulette(self, apis: List[str], k: int, dimension: str = "") -> List[str]:
        ...
        for _ in range(k):
            if not pool:
                break
            scores = [self._score(a, dimension) for a in pool]
            total = sum(scores)
            if total == 0:
